...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 51 | 随机 200 组燕尾闭合 ≤ 0.1mm 与销-齿交替互补（mulberry32 固定种子）、直榫 10 组手工核算、三视图一致性、销板侧视斜度/半齿销/半隐槽底、边界警告、锯切线数量、导出导入往返全等、导入缺关键项报错点名/缺可补项按默认补全、性能 |
| 组件（RTL） | 12 | 新建流程、键盘 ±0.5 微调、脏参数提示条、类型切换联动、列表筛选、非法导入错误提示（含缺板厚/板B 不写库）、缺项导入按默认补全提示、表单钳制 |
| E2E（Playwright） | 8 | 全流程（新建→出图→保存→列表）、警告场景、键盘微调、导出→删除→导入 viewBox 一致、导入缺关键项被拒绝且不写库、打印校验尺、五类型出图、知识卡 |
