...
| 方案库 | localStorage 保存方案，按类型/板厚筛选，支持 JSON 导出/导入（往返完全一致；缺关键项点名报错，木料/配合/锯路等可补项按默认补齐并提示） |
...
- 边齿布局：槽宽 = 齿根宽，边距 = 半齿根宽
- 销板互补：n 齿 → n+1 销，整销落进齿间槽（销/槽边界在 0.1mm 网格上逐格重合），两端为半齿销；贴合面宽、背面窄，整销每边收窄 tB/r
- 半隐燕尾：齿深 = 0.75 × 板厚；齿顶宽在槽底（最深面）
- 齿根宽低于最小值（软木 6mm / 硬木 4mm）、或整销背面被斜度切没时输出**警告**而非静默放行
...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 49 | 随机 200 组燕尾闭合 ≤ 0.1mm（mulberry32 固定种子）、销板互补齿形（n+1 销/整销入槽/半齿销/贴合面宽背面窄）、直榫 10 组手工核算、三视图一致性、边界警告、锯切线数量、导出导入往返全等、缺关键项点名报错/可补项按默认补齐、性能 |
| 组件（RTL） | 12 | 新建流程、键盘 ±0.5 微调、脏参数提示条、类型切换联动、列表筛选、非法导入错误提示、缺板厚不写库、缺木料补齐提示、表单钳制 |
| E2E（Playwright） | 7 | 全流程（新建→出图→保存→列表）、警告场景、键盘微调、导出→删除→导入 viewBox 一致、打印校验尺、五类型出图、知识卡 |
